...
{
    public partial class DeploymentHealthCheck : Form
    {
        string formText;
        ...
        public DeploymentHealthCheck(string applicationName)
        {
            InitializeComponent();
            comboBxAppList.Items.Add(applicationName);
            comboBxAppList.Text = applicationName;
            listViewResources.DoubleBuffered(true);
        }
        ...
        {
            if (comboBxAppList.Items.Count == 0)
            {
                LoadApplications();
            }
            formText = this.Text;
